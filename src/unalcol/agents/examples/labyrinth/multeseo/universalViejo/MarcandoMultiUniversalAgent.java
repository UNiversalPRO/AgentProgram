package unalcol.agents.examples.labyrinth.multeseo.universalViejo;
...
public class MarcandoMultiUniversalAgent extends MarcandoMultiTeseoAgentProgram {
	...
	public MarcandoMultiUniversalAgent() {
	...
	public MarcandoMultiUniversalAgent(boolean id) { // AGENTE DE PRUEBAS
		memory = new TreeMap<Integer, TreeMap<Integer, Integer[]>>(); // [PF, PD, PA, PI]
		G = new TreeMap<Node, TreeMap<Node,Integer[]>>();
		ways = new LinkedList<Integer>();
		completeNodes = new TreeSet<Node>();
						...
						if ( memory.get(posX).get(posY)[i] == j ) {
							...
							if (preX != tmpFuture[0] || preY != tmpFuture[1]) { // No devolverse en el camino que iba
								...
								if (agent[i]) { // se devuelve por el camino en el que estaba
									for (int l = compass + 1, k = 0; k < 4; l = (l + 1) % 4, k++) {
...
//		System.out.println("orientacion ="+orientation+" compas = "+compass);
		memory.get(posX).get(posY)[orientation]++; // marca el camino cuando sale de la casilla actual
		...
		memory.get(tmpFuture[0]).get(tmpFuture[1])[(orientation + 2) % 4]++;
